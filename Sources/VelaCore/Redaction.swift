...
public struct RedactionRule: Identifiable, Codable, Equatable, Sendable {
    ...
    public init(id: UUID = UUID(), name: String, pattern: String, isEnabled: Bool = true, isBuiltIn: Bool = false) {
        self.id = id
        self.name = name
    ...
    public static func builtIns() -> [RedactionRule] {
        [
...
public struct RedactionSpan: Codable, Equatable, Hashable, Sendable {
    ...
    public static let notInMessageBody = -1
    ...
    public var isInMessageBody: Bool { location != Self.notInMessageBody }

    public init(ruleName: String, location: Int, length: Int) {
...
public struct RedactionResult: Equatable, Sendable {
    ...
    public var spans: [RedactionSpan]
    public var didRedact: Bool { !spans.isEmpty }

    public init(text: String, spans: [RedactionSpan]) {
        self.text = text
        self.spans = spans
    }
}
...
public struct Redactor: Sendable {
    public var rules: [RedactionRule]
    ...
    public static func invalidRuleIDs(in rules: [RedactionRule]) -> Set<UUID> {
        var bad: Set<UUID> = []
    ...
    /// mangled sentence.
    public static func marker(for ruleName: String) -> String { "[redacted: \(ruleName)]" }
}
...
public enum EgressPolicy {
    ...
    public static var isLocalOnly: Bool {
