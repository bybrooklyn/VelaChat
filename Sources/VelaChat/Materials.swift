...
extension View {
    ...
    /// A flat, app-tinted text field background — for use with
    /// `.textFieldStyle(.plain)` instead of `.roundedBorder`.
    /// `.roundedBorder`'s focus ring is drawn by AppKit directly and does
    /// *not* follow SwiftUI's `.tint()` (confirmed: this app already
