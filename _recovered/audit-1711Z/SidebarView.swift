...
struct SidebarView: View {
    ...
    var body: some View {
        ...
        .frame(minWidth: 250, idealWidth: 274)
    ...
    /// A single, unambiguous full-width button — not a tinted row easily
    /// mistaken for a label. Solid fill so it reads as the sidebar's one
    /// primary action, matching the weight `settingsFooter` has at the
    ...
    /// Routes taps through `selectConversation` (provider/thinking/section
    /// sync) instead of writing `activeConversationID` directly, and ignores
    /// `nil` so the sidebar never lands in a fully-deselected state.
    private var selectionBinding: Binding<UUID?> {
        Binding(
            get: { appModel.activeConversationID },
            set: { newValue in
                guard let id = newValue, let conversation = appModel.conversations.first(where: { $0.id == id }) else { return }
                if renamingConversationID != nil, renamingConversationID != id {
    ...
    private var pinnedConversations: [Conversation] { appModel.conversations.filter(\.isPinned) }
    private var unpinnedConversations: [Conversation] { appModel.conversations.filter { !$0.isPinned } }
    ...
    private var searchResultsList: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                Text(results.isEmpty ? "No matches" : "\(results.count) result\(results.count == 1 ? "" : "s")")
    ...
    private func row(for conversation: Conversation) -> some View {
        ...
        // Without this, List's own default row insets sit outside the row's
        // custom padding, so the fill/outline rectangle it draws is narrower
        // than the row's real selectable width — a visibly wrong-size outline.
    ...
    var body: some View {
        Button(action: action) {
            ...
            .overlay {
                RoundedRectangle(cornerRadius: Theme.Radius.row, style: .continuous)
                    .stroke(
                        selected ? Color.clear : (isHovering ? Theme.controlStroke : Color.clear),
                        lineWidth: 1
                    )
            }
...
private struct ConversationRow: View {
    ...
    var body: some View {
        HStack(spacing: 9) {
            Image(systemName: conversation.messages.isEmpty ? "bubble.left" : "text.bubble")
                .font(.system(size: 12, weight: .medium))
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 7)
        // Selected rows are filled; everything else is a hairline outline that
        // only appears on hover, so the list reads as one calm column instead
        // of a stack of competing filled boxes.
        .background(
            selected ? Theme.sidebarSelection : Color.clear,
            in: RoundedRectangle(cornerRadius: Theme.Radius.row, style: .continuous)
        )
        .overlay {
            RoundedRectangle(cornerRadius: Theme.Radius.row, style: .continuous)
                .stroke(
                    selected ? Color.clear : (isHovering ? Theme.controlStroke.opacity(0.9) : Color.clear),
                    lineWidth: 1
                )
        }
        .contentShape(Rectangle())
